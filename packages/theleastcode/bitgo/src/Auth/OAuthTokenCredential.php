...
use Exception\BitgoConnectionException;
use Handler\IBitgoHandler;
use Rest\ApiContext;
use Security\Cipher;
 ...
 */
class OAuthTokenCredential extends BitgoResourceModel
{
     ...
     */
    public static $AUTH_HANDLER = 'Handler\OauthHandler';
    ...
    {
        $httpConfig = new BitgoHttpConfig(null, 'POST', $config);
     ...
     * @return null
     * @throws BitgoConnectionException
     */
    private function generateAccessToken($config, $refreshToken = null)
    {
        ...
        if ($response == null || !isset($response["access_token"]) || !isset($response["expires_in"])) {
            ...
            BitgoLoggingManager::getInstance(__CLASS__)->warning("Could not generate new Access token. Invalid response from server: ");
            throw new BitgoConnectionException(null, "Could not generate new Access token. Invalid response from server: ");
        } else {
