...
class ApiContext
{
     ...
     * If the value is not set, it would get the value from @\Core\BitgoCredentialManager
     *
     * @var \Auth\OAuthTokenCredential
     */
    private $credential;
     ...
     * Construct
     *
     * @param \Auth\OAuthTokenCredential $credential
     ...
     * Get Credential
     *
     * @return \Auth\OAuthTokenCredential
     */
    public function getCredential()
    {
        if ($this->credential == null) {
            return BitgoCredentialManager::getInstance()->getCredentialObject();
        }
    ...
    {
        $result = BitgoConfigManager::getInstance()->get('http.headers');
        $headers = array();
    ...
    public function addRequestHeader($name, $value)
    {
        ...
        BitgoConfigManager::getInstance()->addConfigs(array($name => $value));
    }
     ...
     * Sets the request ID
     *
     * @param string $requestId the Bitgo-Request-Id value to use
     */
    ...
    /**
     * Resets the requestId that can be used to set the Bitgo-request-id
    ...
    {
        BitgoConfigManager::getInstance()->addConfigs($config);
    }
    ...
    {
        return BitgoConfigManager::getInstance()->getConfigHashmap();
    }
    ...
    {
        return BitgoConfigManager::getInstance()->get($searchKey);
    }

    /**
     * Generates a unique per request id that
     * can be used to set the Bitgo-Request-Id header
